...
if [ "X${HOST_ADDR}" = "X" ];then
  HOST_ADDR=$(cat cmd/bmsitter/main.go | grep "@host" | awk '{print $3}')
  HOST_ADDR=$(echo  ${HOST_ADDR} | cut -d ':' -f 1)
else
    sed -i "s/@host .*:8080/@host ${HOST_ADDR}:8080/g" cmd/bmsitter/main.go
fi

# generate api docs
swag init -g cmd/bmsitter/main.go --parseDependency --parseInternal
checkResult $?
